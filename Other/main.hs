...
separa ((x,y,z):coda) = (x:listax, y:listay, z:listaz)
  where (listax, listay, listaz) = separa coda1
  
-- Esercizio 49

diversi :: Eq a => [a] -> Bool
diversi [] = True
diversi (testa:coda) = not (elem testa coda) && diversi coda1
...
phrase :: Syntree -> String
phrase (Node symbol []) = " " ++ symbol ++ " "
phrase (Node symbol nodes) = concat [phrase node | node <- nodes]

-- Esercizio 52

scindi_quadruple :: [(a,b,c,d)] -> ([d],[c],[b],[a])
scindi_quadruple lista = ([w | (_,_,_,w) <- lista], 
                          [z | (_,_,z,_) <- lista], 
                          [y | (_,y,_,_) <- lista], 
                          [x | (x,_,_,_) <- lista])
...
scindi_quadruple [] = ([],[],[],[])
scindi_quadruple ((x,y,z,w):coda) = (w:listaw, z:listaz, y:listay, x:listax)
  where (listaw, listaz, listay, listax) = scindi_quadruple coda1
...
valore _ (Const n) = n
valore s (Indexing name e) = (cerca name stato) !! (valore s e)
valore s (Plus e1 e2) = (valore s e1) + (valore s e2)

-- Esercizio 55

flat :: Tree -> [Int]
flat (Node n children) = [n]++(concat(map flat children))

minmax :: Tree -> (Int,Int)
minmax t = (minimum (flat t), maximum (flat t))

-- Esercizio 56

torneo :: [(String,String)] -> [((String,String),(String,String))]
torneo coppie = [(x,y) | x <- coppie, y <-coppie, x /= y]

-- Esercizio 57

type Figura = [String]

sovra :: String -> String -> String
sovra line1 line2 = [if c1 == '#' || c2 == '#' then '#' else '.' | 
                                                      (c1,c2) <- zip line1 line2 ]
sovrapposizione :: Figura -> Figura -> Figura
sovrapposizione fig1 fig2 = [sovra line1 line2 | (line1,line2) <- zip fig1 fig2 ]
...
mat1 +++ mat2 = [ zipWith (+) line1 line2 | (line1,line2) <- zip mat1 mat2 ]
...
match [] (Node _ _) = False
match [c] (Node c' []) = c == c'
match (c:coda) (Node c' figli) = c == c' && length([f | f <- figli, match coda f]) > 0

-- Esercizio 61

data Albero = Nodo String [Albero]

inverso :: Albero -> Albero
inverso (Nodo s figli) = (Nodo (reverse s) [inverso f | f <- figli])

-- Esercizio 62

data Albero = Nodo [Int] Albero Albero

data Albero' = Nodo' Int Albero' Albero'

riduzione :: Albero -> Albero'
riduzione (Nodo numeri f1 f2) = (Nodo' (sum numeri) (riduzione f1) (riduzione f2))
