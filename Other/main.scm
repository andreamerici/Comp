...
(define (sequenza x n)
  (if (= n 0) '() 
      (append (sequenza x (- n 1)) (list (power x n)))))
...
(define (element M i j)
  (if (equal? (indexing M i) 'exception)
      'exception
      (indexing (indexing M i) j)))

; Esercizio 52

(define (superlist S L)
  (if (null? S) #f
      (if (null? L) #t
          (if (not (equal? (car S) (car L))) #f
              (superlist (cdr S) (cdr L))))))
...
(define (fun n f g)
  (if (= n 0) '()
     (append (fun (- n 1) f g) (list (+ (f (g n)) (g (+ n 1)))))))

; Esercizio 56

(define (cascata F n)
    (if (null? F) n
        ((car F) (cascata (cdr F) n))))
...
(define (coppie L)
  (if (null? L) '()
      (if (null? (cdr L)) (list L)
          (cons (list (car L) (cadr L)) (coppie (cddr L))))))
...
(define (sorted? lista)
  (if (or (null? lista)(null? (cdr lista))) #t
      (if (< (car lista)(car (cdr lista))) (sorted? (cdr lista))
          #f)))
...
(define (somma lista)
  (if (null? lista) 0
      (+ (car lista) (somma (cdr lista)))))

(define (bilancio? lista)
  (= (car (reverse lista)) (somma (cdr (reverse lista)))))