...
visualizza_mossa(X, Y) :- writeln([muovi, un, disco, dal, piolo, di, X, al, piolo, di, Y]).
...
fibPair(0,pair(0,1)) :- !.
fibPair(N,pair(X,Y)) :- N1 is N-1,
                        fibPair(N1,pair(U,V)),
                        X = V, Y is U+V.
                        
fastFib(N,F) :- fibPair(N,pair(F,_)).

% Setaccio di Eratostene

primi(N,Primi) :- numeri(N,Numeri), setaccio(Numeri,Primi).

numeri(2,[2]) :- !.
numeri(N,Numeri):- N > 2, N1 is N-1, numeri(N1,Numeri1), append(Numeri1,[N],Numeri).

setaccio([],[]) :- !.
setaccio([P|TN],[P|TP]) :- rimuovi_multipli(P,TN,R), setaccio(R,TP).

rimuovi_multipli(_, [], []):- !.
rimuovi_multipli(P, [N|TN], R) :- 0 is N mod P, rimuovi_multipli(P,TN,R), !.
rimuovi_multipli(P, [N|TN], [N|TR]) :- rimuovi_multipli(P,TN,TR).

% Espressioni aritmetiche

stato([val(a,1),val(b,2),val(c,3),val(d,4)]).

valore([H|T],V) :- istanza(H,IH), valore_accumulato(IH,T,V).

istanza(X,I) :- stato(S), member(val(X,I),S), !.
istanza(X,X).

valore_accumulato(A,[],A) :- !.
valore_accumulato(A,[+,X|T],V) :- istanza(X,IX), AX is A+IX, valore_accumulato(AX,T,V), !.
valore_accumulato(A,[-,X|T],V) :- istanza(X,IX), AX is A-IX, valore_accumulato(AX,T,V).

% Espressioni pseudo-regolari

% a(*2)b(c|d|ab)(*4)|(ca)(*2)a

regex([alt([[star([a],2),b,star([alt([[c],[d],[a,b]])],4)],[star([c,a],2),a]])]).

match(S) :- regex(R), gen(R,S).

gen([],[]) :- !.
gen([H|TR],[H|TS]) :- gen(TR,TS).
gen([alt(L)|TR],S) :- member(A,L), append(A,TR,ATR), gen(ATR,S).
gen([star(L,N)|T],S) :- kleene(L,N,K), append(K,T,KT), gen(KT,S).

kleene(_,_,[]).
kleene(X,N,K) :- N1 is N-1, N1 >= 0, kleene(X,N1,K1), append(X,K1,K).